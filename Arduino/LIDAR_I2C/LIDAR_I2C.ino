...
void loop() {
  ...
  if(Serial1.available() > 0) {
    ...
    //Serial.write(inByte);
    decodeData(inByte);
    if (dataIndex == 2 && Data_4deg_index == 89) {
      Data_status = 0; Data_4deg_index = 0; Data_loop_index = 0;

      Serial.print(distance_array[100]);
    }
  }
  ...
  if(Serial.available() > 0) {
    ...
    int index = inString.toInt();
    Serial.print("index: ");
    Serial.print(index);
    Serial.print("distance: ");
    Serial.print(distance_array[index]);
    Serial.println();
  }
}
...
void sendData() {
  ...
  int index = inString.toInt();
  String data = String(distance_array[index], DEC);
