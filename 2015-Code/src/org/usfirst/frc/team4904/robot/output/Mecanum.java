...
public class Mecanum implements Updatable {
	...
	public Mecanum(SpeedController frontLeftWheel,
			SpeedController frontRightWheel, SpeedController backLeftWheel,
	...
	private void move(double desiredSpeed, double desiredAngle, double turnSpeed) {
		...
		double frontLeft = desiredSpeed * Math.sin(desiredAngle + Math.PI / 4)
				+ turnSpeed;
		double frontRight = desiredSpeed * Math.cos(desiredAngle + Math.PI / 4)
				- turnSpeed;
		double backLeft = desiredSpeed * Math.cos(desiredAngle + Math.PI / 4)
				+ turnSpeed;
		double backRight = desiredSpeed * Math.sin(desiredAngle + Math.PI / 4)
				- turnSpeed;

		double scaleFactor = Math.max(Math.max(
	...
	public void setDesiredSpeedDirection(double desiredSpeed,
			double desiredAngle) {
