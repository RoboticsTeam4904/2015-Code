...
public class OperatorGriffin extends Operator {
	...
	public synchronized void update() {
		if (stick.buttons[1].get()) adjust(stick.getY()); // When button 2 is pressed, adjust the winch
		...
		if (stick.buttons[11].get()) grab(MODE_THIN_TOTE); // When button 12 is pressed, grab a thin tote
		if (stick.buttons[9].get()) grab(MODE_WIDE_TOTE); // When button 10 is pressed, grab a wide tote
		if (stick.buttons[7].get()) grab(MODE_CAN); // When button 8 is pressed, grab a can
		if (stick.buttons[10].get()) release(); // When button 11 is pressed, release a thin tote
		if (stick.buttons[8].get()) release(); // When button 9 is pressed, release a wide tote
		if (stick.buttons[6].get()) release(); // When button 7 is pressed, release a can
		updateWinch();
	}
}