	...
	public synchronized void update() {
		...
		if (stick.buttons[0].get()) {
			if (grabber.getState() == Grabber.GrabberState.OPEN || grabber.getState() == Grabber.GrabberState.DISABLED) {
				logger.v("Nachi Tote Grab");
				toteGrab();
			} else {
				logger.v("Nachi Tote 0 Release");
				release();
			}
		}
		// When button 2 is pressed, can grabbing
		if (stick.buttons[1].get()) {
			if (grabber.getState() == Grabber.GrabberState.OPEN || grabber.getState() == Grabber.GrabberState.DISABLED) {
				logger.v("Nachi Can Grab");
				canGrab();
			} else {
				logger.v("Nachi Can Release");
				release();
			}
		}
		...
		if (stackingStep > 0) {
			...
			switch (stackingStep) {
				...
				case 6: // Wait for grabber to close.
					if (grabber.getState() == Grabber.GrabberState.CLOSED_TOTE || grabber.getState() == Grabber.GrabberState.CLOSED_CAN) {
						stackingStep = 7;
					}
					break;
			...
			// When button 10 is pressed, stack a tote
