...
public class IMU extends SensorBase implements PIDSource, LiveWindowSendable, Runnable {
	...
	protected volatile double lastYPRUpdate;
	protected volatile float yawRate;
	protected volatile float pitchRate;
	protected volatile float rollRate;
	protected volatile float lastYaw;
	protected volatile float lastPitch;
	protected volatile float lastRoll;
	...
	protected int decodePacketHandler(byte[] received_data, int offset, int bytes_remaining) {
		...
		if (packet_length > 0) {
			...
			double timeDiff = Timer.getFPGATimestamp() - lastYPRUpdate;
			yawRate = (float) ((yaw - lastYaw) / timeDiff);
			lastYPRUpdate = Timer.getFPGATimestamp();
			lastYaw = yaw;
			SmartDashboard.putNumber("yawspeed", yawRate);
		}
